...
- Current narrowed boundary:
  tele promoted target-2 records are now proven to enter concrete `0x2416d0` selected-index vectors, the small admitted promoted sets reach `(state=5,target=2)` stores, watched state-5 records continue into `0x244560 -> 0x25d090`, four-zoom runs prove `0x2457c0` materializes finite non-sentinel coordinate pairs into `state+0x1e8` from the state-5 store path, and representative emitted pairs are read by `0xe8e70` vector-copy work under both State-helper copy-out paths; `0x25d090` output/effect, copied coordinate-vector destination/effect, downstream image effect, final acceptance/rejection, and reducer closure remain open.
- Consequence:
