...
| Tele odd-camera routing, especially C6 | 8 | 7 | 8 | 23 | 3 | **7.67** | Strong tele-risk; keyed-helper/vector-builder and projection field-pack dispatcher boundaries are excluded, and both the direct candidate loop and stereo-side keyed-record loop now prove key `15` is filtered by `object+0x30 = 0`; remaining work is narrower alternate-route or terminal-filter proof |
...
- Reason:
  it already has concrete exclusions plus two proven filter points: the visible-`src1` keyed helper / vector-builder boundary and projection field-pack dispatcher boundary are excluded as positive C6 routes, and the direct candidate loop plus stereo-side keyed-record loop both filter key `15` by `object+0x30 = 0` under canonical bridge HDR runs; final closure still needs alternate-route or terminal-filter proof.
- Consequence:
...
- Nature:
  alternate-route or terminal-filter proof after the now-proven `object+0x30 = 0` direct and stereo-side gates
- Dependency note:
