...
| A (prefusion-reducer-static) | src1/src2 reducer roles + reduction operator (CLM-PREFUSION-002) | **Predict-then-verify, finder+independent-verifier.** Arg map: `src1=cache+0x238=arg1` (geometry ANCHOR, warp coords clamped to its dims), `src2=cache+0x248=arg2` (geometry/ROI box; `0x374ac0` margin-ZERO-FILLS src2's own buffer, NOT a sampling clip). Reduction operator = **1/Σscore-normalized weighted average** (`rcpss 0x36a938`; H1 "src1/src2=value/weight pair + per-pixel divide" REFUTED). **Contributor gate:** `0x36930f` `0x80000000` = index-validity sentinel (per-contributor tile skip); `0x36cde0` score stored UNCONDITIONALLY (`0x369e91`) — weights, doesn't gate; loop N = vector span `>>7 /5`. **Score:** `0x36cde0=sqrtss(x*y)`, SSIM `0x5fdc50={.01,.03,.03,1}` floor −0.8 renorm 1/0.19. **lane-3** = `recip*0.2` per-pixel gain on ±0.1-clamped correction → forced 1.0. **Caught+corrected a subagent fabricated "zero rcpss" negative.** | `5a6c266`→`a75363b` |

| D (final-acceptance-static) | final merge accept/reject terminal filter (extends Codex sentinel frontier) | **OBSERVED:** `0x218b30` is a STATISTICS reducer (mean clamped score→`*(%r14)`; threshold-exceed fraction→ret xmm0), NOT a record materializer. Per-pair accept = both lanes strictly >0 (`0x218bc4 jae`, `0x218bd3 jbe`); sentinel `(-1,-1)` rejected. `<8` positive pairs (`0x2170d4 jl 0x217d00`) → CLEAN early return skipping the whole merge body (not a throw). **Real N→1 accumulate candidate = `0xe6ba0`** (≥8 path ~`0x2170da`) — wave-2 target. | `a75363b` |

| B2 | LRI per-camera calibration origins (Blocker 2, input side) | **OBSERVED (byte-verified):** intrinsics live in LRI **Block 3** (smallest 16×field-13); cam0 `fx=fy=3375.884, cx=2084.516(≈4160/2), cy=1541.342(≈3120/2)`; 16 cams cluster into the L16 **5+5+6** focal tiers (fx ≈3370/8300/18700). Stored as tagged protobuf fixed32. **CANDIDATE** (subagent, not all re-verified): 3×3 K layout, distortion coeffs `[0.0326,0.1501,0,0,-0.5774]`, 101+30 LUT, calib date. LRI-side only (binary consumer = Codex's `0x23faf0` thread, untouched). **Four-zoom+two-unit (`four_zoom_two_unit.md`):** the 16-cam block + 5+5+6 tiers hold across all 4 zooms AND both units; intrinsics are per-body constants (identical across a unit's zooms); Unit-1 fx`[3376,8283,18795]`≠Unit-2`[3373,8281,18684]`; block index varies (blk3/blk4). **Field map (`verified_field_map.md`):** K `[fx 0 cx;0 fy cy;0 0 1]` per-scale, 5-coeff distortion, 2 LUTs, calib date — OBSERVED structure. **Cross-unit (`cross_unit_values.md`):** K+distortion differ per body, identical schema. **Block 6 (`block6_color_candidate.md`):** 42-rec block, two 3×3 row-sum-(0.964,1.0,0.825) matrices = CANDIDATE color-calibration (per-body constant, Unit-1≠Unit-2, four-zoom); plausible LRI origin of the A5 runtime color matrix. **Unknowns (`calibration_unknowns_block6.md`):** `f3.2.1`(818/1500)+`f3.2.4`(60/62) CONSTANT across 16 cams (not focal); `f3.2.6` clusters cams 0–4 (7900–10200) vs 5–15 (1370–2300); f3.3.2.5 LUT y is NON-monotone (0→31.65@idx72→29.7, verifier-corrected). | `laneB2_lri_calibration_origins/` |
