...
> **"Graduated" / "resolved" ≠ TRUE.** They mean a finding was brought to the rigor that makes it worth
> Codex's validation time — every one is `NEEDS_CODEX_VALIDATION`, a candidate, not a verdict. The
> orchestrator is NOT the final verdict on truth; Codex validates. "Complete" below = the investigation
> reached that rigor on all 80 rows, NOT that any of them is confirmed truth.
...
until its per-tier (28/35/70/150) data is captured + verified. **Current truth: 61 / 80 graduated** (+14 subsumed, +3 subsumed-corrected, +1 superseded, +1 retracted = **80/80 RESOLVED; 0 STAGING**). The tractable remediation set is COMPLETE — every 2-day quarantine doc is graduated, subsumed-into-a-graduated-parent, superseded, or retracted. **The 19 non-graduated rows are themselves brought to validate/invalidate rigor in `../opus_findings_for_codex/RESIDUAL_VALIDATION_LEDGER.md` (7 INVALIDATED with the alternative investigated+graduated, 12 VALIDATED with all 6 shed sub-mechanisms DECODED byte-exact in `parked_residuals_decoded_FOURZOOM.md` — none parked; 2 further wrong claims caught; merge SELECTION prong `0x36930f` closed).** Remaining work = **Codex validation** + explicitly-scoped NON-blocking sub-residuals (CCM `eax==0` writer store; merge coverage-sentinel `0x36930f` runtime; row77 firmware-generation hypothesis + 182-file unit-assignment; other-stage U2 runtime; lens-shading {1,15} on U2) — all tracked, none parity-blocking. Jump 28→60 = 2026-06-04: tool wall BROKEN (W5+W5b → 4-tier merge/score/CCM magnitudes + weight_vec4 + reducer two-prong); 13 static byte-verified; LRI 8-seed two-unit intrinsics; 15 subsumption-confirmed (→`SUBSUMPTION_RESIDUALS.md`); C-group runtime 6 (`cgroup_runtime_FOURZOOM`, corrections L2-4 FIRES + gate2/3 FIRE; 70mm dispatcher native re-measured 220/266/246); color-consumption 3 (`color_consumption_FOURZOOM`: per-cam CCM `0xa9f20`=Block-6 f2.2 four-zoom THEN I1I2I3 `0xbfa20`; AWB=1/gain demosaic-folded four-zoom). **Unit-2 RUNTIME universality CLOSED** (`unit2_runtime_universality_FOURZOOM`): merge/score/CCM/I1I2I3 mechanisms hold on body-2 (223961c6), structure universal + values per-body. **Only remaining: row77** (needs a 182-class unassigned LRI not in the corpus — genuinely corpus-blocked, not method-blocked) + narrowed opens (CCM eax==0 writer store; sentinel-gate runtime). Earlier: W1:2 + W1b:4 + W3:14 + W3b:2.
...
| 34 | → GRADUATED(color_consumption_FOURZOOM §3) | **GRADUATED** | AWB=1/gain folded into demosaic, four-zoom: 1/R→0.125 saturates R, G/B flat (35/70/150 R-factor 3.82/4.35/4.54×; 28 prior); site 0xa9340 divss triple | Crender |
...
| 77 | laneP_parser_gap_182/ (residual_alternate_container + observations) | **GRADUATED** | NOT corpus-blocked: 182 unassigned deterministically parsed (O1–O6, reproducible) — ≤4 blocks {0:4,1:10,2:26,3:77,4:65} vs assigned 11–12; no third unit; spot-re-verified (L16_00795=1 block). Hypothesis+assignment=scoped opens, non-blocking | Blri |
