...
| M14.1 | **Composite-anchor pre-fusion kernel at `libcp+0x2b3410..0x2b3448`** — 4-input × weight SIMD accumulator (classic `movaps + mulps + addps + movaps` 4-way weighted blend). Consumes the 5 A-cam RIC L0 buffers (A1..A5 at 28mm) to build the `src1`/`src2` composite IGs that the IRAMP body at `0x365960` takes as its first two ImageGenerator inputs. Called from IRAMP body at offset `+0x5f0` (`libcp+0x365f50`). **This is the kernel that consumes "dropped" cams** — they feed the composite anchor src1/src2 pre-fusion upstream of IRAMP. Cross-zoom universality DIRECTLY CONFIRMED 2026-04-20 v2.1.3: direct BP hit counts = **28mm: 20,351 WP trips at hottest PC 0x2b341e (30% of render); 70mm: 1,736,345 BP hits (partial render before SIGABRT); 150mm: 1,538,833 BP hits (partial render before SIGABRT)**. The kernel fires with multi-million hits at 70mm and 150mm, confirming the composite-anchor mechanism is used at ALL zoom tiers, not just 28mm. Refines M13 (`composite_anchor_n1_reducer.md` said src1/src2 wrap composite anchors; this locates the assembly kernel). | 28mm: Session 1 HW watchpoints `/tmp/l16_open_audit/session1/phase2_watchpoints.log` (102,361 trips, "TRULY NOVEL: (none)"). 70mm: `/tmp/l16_open_audit/minimal_70mm.log` (L16_03434 2020-07-14). 150mm: `/tmp/l16_open_audit/minimal_150mm.log` (L16_02285 2018-07-07). Closes §4 OPEN-DROPPED-CONSUMER including cross-zoom extension. | ✅ cross-zoom (3 tiers confirmed) |
