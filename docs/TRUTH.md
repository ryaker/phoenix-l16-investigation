...
| M2.1 | **WarpField 80-byte layout fully decoded 2026-04-21** via runtime LLDB on L16_02130 28mm bridge HDR. Layout: `+0x00..+0x0F` = vec4f homography col 0 (coefs of `dst_x × sample`); `+0x10..+0x1F` = vec4f col 1 (coefs of `dst_y × sample`); `+0x20..+0x2F` = vec4f col 2 (coefs of `sample`); `+0x30..+0x3F` = vec4f col 3 (constant/translation, slot-3 always +1.0 homogeneous marker); `+0x40..+0x47` = ptr to **shared** auxiliary sampled-image struct (ALL 5 warps share one aux image per render; aux.stride@+0x18, aux.data@+0x20); `+0x48..+0x4B` = float X-scale dst→aux-sampling; `+0x4C..+0x4F` = float Y-scale dst→aux-sampling. Per-pixel math: `sample = aux.data[int(dst_y × sy) × aux.stride + int(dst_x × sx)]; out = sample·col2 + col3 + (dst_x·sample)·col0 + (dst_y·sample)·col1; (source_u, source_v) = round(out[0..1] / out[2])`. Inner loop at `libcp+0x366b75..+0x366d50` has the LEA+mulps pattern per offset; perspective divide at `+0x366d19..+0x366d50`. Runtime 28mm L16_02130: all 5 warps share aux ptr `0x600002a40090` (stride=4160, floats ≈ 693-709 — likely depth/disparity buffer); sx=sy=1.0 on all 5 warps (identity canvas-space sampling at 28mm); col0[0] ≈ col1[1] ≈ 2.5 ≈ 10432/4160 (canvas-to-sensor scale ratio); warps[3]=B4 has near-exact diagonal (≈ canonical anchor). | `/tmp/l16_open_audit/warpfield_decode/entry_dump_raw.txt` (3 hits × 5 warps, 471 lines); `/tmp/l16_open_audit/warpfield_decode/warpfield_loads.txt` (annotated disasm slice `0x366b20..0x368ba7`); `tools/libcp_disasm_intel.txt` lines 835819..837634. Closes M2 byte-level extension. | ✅ (L16_02130 28mm only; 70/150mm layout extrapolated, sx/sy likely differ) |
| M2.2 | **PipelineCache offsets for IRAMP dispatch (2026-04-21).** Disasm at `libcp+0x3ec7ac..0x3ec7da` (IRAMP dispatcher, parent of body `0x365960`) loads the IRAMP-body args from the caller's `PipelineCache` struct pointed to by `rdi`: `[rdi+0x1e8]` = `float scale` (`movss`); `[rdi+0x238]` = `src1` IG ptr (`mov`); `[rdi+0x248]` = `src2` IG ptr; `[rdi+0x258]` = `&warps` (`vector<WarpField>`, `lea`); `[rdi+0x270]` = `&srcs` (5-contributor `vector<ImageGenerator>`, `lea`). Aux-image shared_ptr candidates at `[rdi+0x1d8]`/`[rdi+0x1e0]` per PipelineCache dtor at `libcp+0x3eafc1` (walks `+0x1d8/+0x1e0/+0x240/+0x250/+0x258/+0x260/+0x270/+0x278/+0x1d8/+0x188/+0x198` with shared_ptr free_tiny calls). | `/tmp/l16_open_audit/aux_image_decode/aux_dump.txt` (dispatch disasm); `/tmp/l16_open_audit/aux_image_decode/probe_aux.py`; `tools/libcp_disasm_intel.txt`. | ✅ (L16_02130 28mm only) |
| M2.3 | **Aux-image characterization (2026-04-21).** `warps[i]+0x40` points at a shared (ALL 5 warps → same ptr) 64-byte opaque POD struct with: `+0x08/+0x10/+0x18` = packed `Vec2<int32>{width=4160, height=3120}`; `+0x20/+0x28` = data ptr (dup alias); rest zeros. Data buffer = **4160×3120 f32 = 52 MB** at sensor-native, stride = 4160 floats (= 16640 bytes), row-major. MALLOC_LARGE-zone allocation. **Owned by PipelineCache** (dtor at `0x3eafc1` frees it). **Written ONCE upstream of IRAMP setup** — 0 HW-write-WP trips on `aux.data[0]` across 300 IRAMP invocations. Values at L16_02130 28mm: range `[692.75, 739.13]`, mean 710.8, smooth spatial variation but **NOT a radial distance LUT** (samples at col 0/1040/2080/3120 within row 0 give 705/1356/2342/1735 — non-monotonic, non-symmetric about center). Candidates for semantic role: photometric-gain map, per-cam disparity prior, per-pixel warp modulator. Unknown pending aux-writer decode. **WarpField math reinterpretation:** aux acts as a per-pixel SCALAR modulator of the projective row coefs — `out = aux·col2 + col3 + dst_x·aux·col0 + dst_y·aux·col1` — not just a sample-lookup index. **CANNOT be stubbed** with zeros/ones for spike reimplementation (formula collapses). | `/tmp/l16_open_audit/aux_image_decode/aux_dump.txt` (full struct decode + 256-float sample + per-row address analysis); `/tmp/l16_open_audit/aux_image_decode/wp_trips.txt`; `/tmp/l16_open_audit/warpfield_decode/warpfield_loads.txt`. | ✅ (L16_02130 28mm only; semantic role OPEN) |
...
| M13 | **Refined 2026-04-21:** src1/src2 IGs are two sibling-vtable views over an EXTERNAL-PRODUCER `ImageGenerator<uint16_t>` that represents the anchor camera (A1 at 28mm, B4 at 70mm) — vtables `0x65f668` and `0x65f6e8` at `PipelineCache+0x238/+0x248` control different pyramid-tier lookup paths into the same external IG. "Composite" in the IG name is a **pyramid-tier wrapper semantic**, NOT a "blended-from-N-cams" semantic. **No pre-fusion N→1 combiner kernel exists inside libcp.dylib.** Literature audit (2026-04-21, `/tmp/l16_open_audit/composite_combiner_audit/`) confirmed via 9-stage custody chain that the actual 10-cam combiner lives OUTSIDE libcp (most plausibly `libImageAPI.dylib`'s `ImageGenerator<uint16_t>::getTile()`). Inside libcp, IRAMP body at `0x365960` is the per-tile N→1 reducer (M1 smoking-gun accumulator at `0x369fa1`); src1/src2 arrive ALREADY-WRAPPED from the external producer. Per-tile op at vt[+0x30] = `0x3ecc10`/`0x3ecd80`; vt[+0x18] never fires. Rooted at `PipelineCache+0x170` = `lt::ReferenceImageCache`. | `composite_producer.md`, `merge_function_reconciliation.md` (9-stage external-producer argument), `runreferencegroupcams_body.md` (intra-libcp combiner candidates refuted), `stackfusion_characterization.md` (StackFusion 0-hits on bridge), `/tmp/l16_open_audit/composite_combiner_audit/scratch_file_summary.csv`. LLDB L16_02130 28mm + L16_00010 70mm. | ✅ (2 LRIs only; external producer NOT yet disassembled) |
| M14 | Per-contributor photometric pre-norm INSIDE IRAMP at `libcp+0x3eced0`. `out = sqrt(max(0, in × IG+0x10))` per-channel SIMD (`mulps+maxps+sqrtps`). Alpha lane = 1.0 via 3× `insertps` with const from `0x5a8128`. IG+0x10 = effective-FOV ratio per C17. Dispatch chain: IRAMP body `0x366f1c` → `0x374ac0` → vt[+0x30] indirect → `0x3eced0`. Hit count: 1 per camera per pipeline invocation at 28mm L16_02130. | `per_camera_radiometric_weight.md`. LLDB read watchpoint on closure+0x30. | ✅ (L16_02130 28mm only) |
| M14.1 | **SUPERSEDED 2026-04-21 — semantic attribution CORRECTED via runtime probe.** The kernel at `libcp+0x2b3410..0x2b3448` is NOT a composite-anchor pre-fusion combiner. It is a **cubic B-spline separable 4-tap resampler inside the Halide `color_denoise_multiplier` stage** (Halide function name captured from runtime closure ASCII string at `rdi+0x28..` on outer function `libcp+0x2b31c0`). Runtime LLDB evidence (2026-04-21 on L16_02130 28mm bridge HDR): (1) weight LUT at `closure[+0x18]` is 4096 B = 64 phases × 16 f32 (= 4 taps × 4 RGBA broadcasts per phase); per-phase tap-sum = 1.000 exactly; midpoint phase 32 = `[0.021, 0.479, 0.479, 0.021]` (canonical cubic B-spline midpoint); phase 0 = `[1/6, 4/6, 1/6, 0]` (B-spline base). (2) 4 input pixel pointers (rax/rcx/rsi/rdi) advance by 0x10 per iteration = adjacent RGBA-vec4 pixels along a scan line. (3) 4 weight pointers (r8/r9/r10/r11) do NOT advance = filter-tap semantics, not per-pixel weighting. (4) Input pixel samples at hit #1 show post-ISP RGBA with alpha=0.833 = already-demosaiced imagery being resampled, not pre-fusion of raw Bayer. (5) 3135 outer invocations per single 28mm render = per-tile-per-pyramid-level call pattern. **Hit counts from v2.1.3 are still factual** (28mm 20K WP trips, 70mm 1.7M, 150mm 1.5M) but consistent with hot pyramid resampler firing per-tile-per-level, not per-render composite-anchor fusion. **Session 1 #15 Q-DROPPED-CONSUMER verdict still holds** (dropped-cam RIC L0 buffers ARE consumed — by this resampler reading their pyramid-cached post-ISP output, NOT by a dedicated blender). **Phoenix posture unchanged** (must run ISP for all fired cams because their pyramids are consumed downstream). **What this invalidates**: the "composite-anchor pre-fusion" story (A1..A5 → src1/src2 blend) does NOT exist as a distinct kernel. M13 still stands structurally (src1/src2 wrap the same anchor cam's pyramid via sibling vtables), but "composite" means "pyramid-tier wrapper" not "blended-from-5-cams". **New OPEN item**: where, if anywhere, is an actual multi-cam combiner for dropped cams? Scratch files (`composite_producer.md`, `anchor_prefusion_and_c6.md`) may describe a different stage, or may suffer the same semantic-attribution error. Needs re-audit. | 28mm runtime probe: `/tmp/l16_open_audit/composite_weights/lut_4096B.bin` (actual 64-phase filter LUT), `/tmp/l16_open_audit/composite_weights/probe_console.log` (closure decode with "color_denoise_multiplier" string hit), `probe_mod.py` (reusable runtime probe module). Superseded via KMS supersede `aae162fe → fc128178`. | 📐 (kernel decoded; semantic role corrected; M13 "composite" meaning under re-audit) |
...
| I9 | `lt::SourceImageCache` (SIC) tree at `RIC+0x30` holds 5 contributor IGs. Node+0x20 = cam_id, node+0x28 = SIC*. SIC vtable `0x65f490`, init `0x3e0330`. LLDB BP at 0x3e0735 captured 5 SIC ptrs with cam_ids {5,6,7,8,9} at 28mm L16_02130. | `refcache_per_camera_isp.md`. | ✅ (L16_02130 28mm only) |
...
| C16 | Per-camera radiometric is distributed across: (a) AWB reciprocal (C1), (b) CCM mired-lerp (C3-C5), (c) vignetting multiply (I5), (d) per-contributor FOV pre-norm `sqrt(max(0, in × FOV_ratio))` at 0x3eced0 (M14). NOT a single radiometric stage. | `per_camera_radiometric_weight.md`, `per_camera_radiometric_weight.md`. | ✅ |
...
| C18 | Per-tile CCM kernel = `ImageConvertColorSpace::$_0` at `libcp+0xbf4a0` (vtable slot 6 of `0x6527c0`), dispatched via `ImageConvertColorSpace` at `0xa9f20` (Halide dispatcher 0x5440). 370 tile-level hits on L16_02130 28mm. Closure capture[3] at `[closure+0x20]` = CCM matrix ptr. **Only 2 distinct CCM matrices** applied across 370 invocations (70 + 300 split). `CCMInterpBetweenCalib` at `0x350bc0` fires 5×, all overwrite the same buffer → single consolidated M per pass. `ImageApplyColorMatrix_3x3_mask` @ 0x300570 and `setColorCorrection_58_Color` @ 0x3466d0 fire 0× on bridge HDR at tested LRIs. | `ccm_factory_to_runtime_transformation.md`, `q12_ics_kernel_28mm.md` (manual-BP re-verify 2026-04-18 on L16_02130: 324 hits / 3 ptr-distinct / **2 value-distinct** CCMs; hit-count delta vs 370 attributed to thread-scheduler serialization under manual stop-continue). | ✅ (L16_02130 28mm only) |
...
| R29 | "JIT wall blocks DemosaicV1 kernel" | **REFUTED Round 2.** DemosaicV1 inner kernel is STATIC SSE2 at `libcp+0x2eef80`. Prior agents misread `std::function::operator()` indirect dispatch as JIT mmap. `demosaicv1_details_cleanup.md`. |
...
| ~~**OPEN-COMPOSITE-COMBINER**~~ | **CLOSED-NEGATIVE 2026-04-21 literature audit.** No dedicated multi-cam combiner kernel exists inside libcp.dylib. The 10-cam merge happens OUTSIDE libcp (most plausibly `libImageAPI.dylib` `ImageGenerator<uint16_t>::getTile()` or Lumen.app body). Inside libcp, IRAMP body at `0x365960` is the per-tile N→1 reducer; src1/src2 (M13) wrap an external-producer IG via sibling vtables. All intra-libcp combiner candidates (`StackFusion 0x1b7d80` = 0 hits on bridge; `FusionCacheBayer::process 0x406a10` = N=1/tile; `0x406970 / 0x1ab2d0 / 0x36fd30 / 0x1bd0a0 / 0x1aab40` — all individually refuted in `runreferencegroupcams_body.md` and `merge_function_reconciliation.md`). Phoenix clean-room implication: Phoenix must IMPLEMENT the external-producer combiner from scratch; disassembly of `libImageAPI.dylib` is a new scoped task (not spike-blocking). | — (closed-negative, not a kernel to find) | — |
| **OPEN-AUX-WRITER** (P1) | **NEW 2026-04-21.** WarpField aux-image (§2.1 M2.3) is written ONCE upstream of IRAMP setup. Writer VA unknown. Aux is a 52 MB f32 buffer at sensor-native dimensions with values [692.75, 739.13] of unknown semantic (not radial distance, not depth range). Without the writer + its math, Phoenix cannot reimplement aux; spike can unblock by LLDB-dumping aux at render time and reading it as a per-zoom-tier constant (not Phoenix-safe, spike-only). | Blocks Phoenix clean-room end-to-end render (Phoenix needs to COMPUTE aux, not dump it). Non-blocking for spike if LLDB-dump path is taken. | LLDB: set earliest-render-setup BP (candidates `libcp+0x3d8780` DepthCache ctor, `0x3eb200` initFusion, `0x3eb3c0` initResAmp). At hit, install HW-write-WP on `PipelineCache+0x1d8/+0x1e0/+0x258` (aux shared_ptr candidates per dtor walk). First WP trip = allocator VA. Then HW-write-WP on `aux.data[0]` for first-fill writer = data-generation stage. Then backtrace + disasm to characterize computation. |
| **OPEN-CCM-NORMALIZATION** | **NEW 2026-04-21 from spike v2 validation on L16_00007 28mm.** TRUTH §2.3 C3 states the kernel formula is `out = M @ (R/G, 1, B/G)` with G forced to 1.0, and §2.3 C5 bit-exactly verified Block 6 field 3 `color_matrix` matches runtime `ctx[+0xdc]`. BUT spike's direct application of the stored matrix produces a heavy green cast (stored rows sum to `(0.54, 1.03, 0.67)` not 1.0; neutral (1,1,1) input → (0.54, 1.00, 0.67) output). Spike empirically found that **row-normalizing M before applying** produces neutral output → there is an implicit normalization step (or equivalent SIMD scaling) between kernel load and apply that TRUTH C3's static disasm did NOT capture. Memory-equivalence verification (C5) doesn't prove byte-exact application. | Blocks Phoenix clean-room CCM parity. Non-blocking for spike (rownorm workaround in `spike_v2/src/utils.py:291`). | LLDB BP at `libcp+0x350c56` kernel entry + exit, dump xmm registers holding M at load-time vs apply-time, diff to detect in-register normalization (`divps` by row-sum or equivalent). ~30 min. |
...
- `composite_producer.md`, `anchor_prefusion_and_c6.md`
...
- `per_camera_radiometric_weight.md`, `per_camera_radiometric_weight.md`
- `ccm_factory_to_runtime_transformation.md`, `ccm_factory_to_runtime_transformation.md`
- `session4_v1_linearize.md`, `demosaicv1_details_cleanup.md`, `demosaicv1_per_camera_dispatch.md`, `demosaicv1_sub_kernels.md`, `demosaicv1_details_cleanup.md`
- `color_pipeline_audit.md`, `refcache_per_camera_isp.md`
...
| #15 cross-zoom extension | not-attempted | **CLOSED (direct evidence).** Minimal BP probes on `libcp+0x2b3410` captured hit counts at 70mm = **1,736,345** (L16_03434) and 150mm = **1,538,833** (L16_02285). Composite-anchor kernel fires with multi-million hits at both zoom tiers = universality DIRECTLY CONFIRMED. Renders died mid-way (SIGABRT, TIFF 0 bytes) but LLDB retained BP counters past target termination. Earlier v2.1.2 claim "BP at 0x2b3410 deterministically crashes render" was a verify-before-trust failure — hit counts were captured, I didn't check the quit-script output line. Corrected here. |
| Instrumentation caveat (revised) | v2.1.2 said BP crashes render | v2.1.3 correction: BP at 0x2b3410 DOES work — hit counts are recorded. Renders still die mid-way under hot-kernel instrumentation (SIGABRT, ~1-2M BP hits tolerable before crash), but the counts survive. At 150mm the S2-probe crash at `libcp+0x2e945d` (`EXC_BAD_ACCESS`) is instrumentation-induced (Rich 2026-04-20: "Lumen ships working 150mm → crash has to be ours"), not a libcp bug. Net: BP-based probes get counts but not full renders when BPs land in hot Halide-JIT kernels. |
| Open items state | 14 RESOLVED / 3 PARTIAL / 0 TRULY OPEN | **17 RESOLVED / 1 PARTIAL (#10) / 0 TRULY OPEN.** All 28/70/150mm bridge HDR spikes architecturally + empirically UNBLOCKED. #10 OPEN-DARKCURRENT remains deferred (not HDR-relevant). |
| Session 2+3 artifacts | — | `/tmp/l16_open_audit/session2_and_3_findings.md` (synthesis), `/tmp/l16_open_audit/session2/phase1_processlevel.log` (11 KB full 70mm render), `/tmp/l16_open_audit/session3/phase1_probe.log` (14 KB partial 150mm before crash), `/tmp/l16_open_audit/minimal_70mm.log` + `minimal_150mm.log` (1.7M + 1.5M 0x2b3410 hit counts — cross-zoom universality proof). |

## 8.4 v2.1.3 → v2.1.4 changelog (2026-04-21 spike v2 validation — TWO major corrections)

Spike v2 ran on L16_00007 (28mm, clean-room Python) and surfaced TWO TRUTH gaps that 5+ prior LLDB sessions had missed. Per Rich's pre-spec validation pattern ("if the spike fails → investigate whether spike code is wrong OR investigation is still wrong — NOT edit doc"), both gaps were root-caused via runtime LLDB probes before this supersession.

| Category | v2.1.3 state | v2.1.4 action |
|---|---|---|
| **§2.1 M14.1 semantic attribution** | "Composite-anchor 4-way SIMD weighted blend at `libcp+0x2b3410` consumes A1..A5 RIC L0 buffers to build src1/src2 composite IGs" | **SUPERSEDED.** Runtime probe (LLDB BP on outer `libcp+0x2b31c0` + closure dump at `rdi+0x28..`) captured Halide's own function-name string `"color_denoise_multiplier"` inside closure. Kernel is a **cubic B-spline separable 4-tap resampler** inside the color_denoise_multiplier stage; weight LUT is 4096 B × 64 subpixel phases with per-phase tap-sum=1.0. 3 independent layers of RE evidence (Session 1 watchpoint traces + static-disasm "4-way SIMD blend" + cross-zoom hit counts) all reinforced the wrong semantic interpretation. Only black-box runtime execution disambiguated. Hit counts and #15 verdict still factual — mechanism reinterpreted. |
| **§2.1 M2.1 WarpField byte layout** | M2 confirmed 80 B total size; field layout partial | **FULLY DECODED.** LLDB probe at IRAMP body setup (`libcp+0x365a36`) captured 5 × 80 B warps on 3 hits × 5 warps = 15 decoded entries. Layout: 4 × vec4f homography columns at `+0x00/+0x10/+0x20/+0x30` + shared aux-image ptr at `+0x40` + X/Y scales at `+0x48/+0x4C`. All 5 warps per render share ONE aux-image (not 5). Per-pixel math: `out = sample·col2 + col3 + (dst_x·sample)·col0 + (dst_y·sample)·col1; (u,v) = round(out[0..1] / out[2])`. Enables Phoenix/spike to implement contributor-to-anchor projection. |
| **M13 ("composite" interpretation)** | "src1/src2 wrap composite anchors; per-tile op at `vt[+0x30]`" — implicit that "composite" = "blended from 5 A-cams" | Refined: "composite" means **pyramid-tier wrapper** (two vtable views of the SAME anchor's pyramid levels), NOT "blended from 5 cameras." No explicit multi-cam combiner kernel has been located. Whether one exists (and under what VA) is a new OPEN item for re-audit. |
| **New OPEN-COMPOSITE-COMBINER** | not yet flagged | Where, if anywhere, is an actual multi-cam combiner for the dropped A-cams (A2..A5) at 28mm? Possibilities: (a) combiner is a separate kernel not yet found; (b) `composite_producer.md`/`anchor_prefusion_and_c6.md` scratch files describe a different stage that hasn't been VA-pinned; (c) dropped cams are consumed only via pyramid-resampling reads by `color_denoise_multiplier` (Session 1 watchpoint traces support this — all trip PCs trace through IRAMP family, consistent with pyramid-level reads rather than dedicated blend). Spike refactor assumes (c) pending audit. |
| **Open items state** | 17 RESOLVED / 1 PARTIAL / 0 TRULY OPEN | **17 RESOLVED / 1 PARTIAL (#10) / 1 NEW OPEN (OPEN-COMPOSITE-COMBINER, non-blocking).** 28mm spike unblocked under assumption that no dedicated combiner exists; visual validation pending. |
| **v2.1.4 spike/probe artifacts** | — | `/tmp/l16_open_audit/composite_weights/lut_4096B.bin` (runtime B-spline LUT), `probe_console.log` (closure-string ASCII dump), `probe_mod.py` (reusable runtime closure-string probe); `/tmp/l16_open_audit/warpfield_decode/entry_dump_raw.txt` (WarpField field-by-field decode), `warpfield_loads.txt` (annotated disasm). Spike workspace: `/Users/ryaker/Dev/L16_Lumen_ReverseEngineering/spike_v2/`. |
| **KMS supersessions** | — | `aae162fe → fc128178` (composite-anchor fact corrected to B-spline resampler). New: `1248d757` (insight: spike validation paid off), `50f9a003` (procedure: LLDB runtime closure-string probe template), `c207d514` (CCM normalization gap). |

## 8.5 v2.1.4 → v2.1.5 changelog (2026-04-21 TRUTH hygiene + OPEN-COMPOSITE-COMBINER closure + aux-image characterization)

| Category | v2.1.4 state | v2.1.5 action |
|---|---|---|
| **Phantom citations** | 8 scratch-file references in TRUTH point at files that do NOT exist on disk (verified deterministically via Python os.path.exists sweep: `composite_anchor_n1_reducer.md`, `demosaicv1_jit_kernel.md`, `per_camera_radiometric_weight.md` [wait — that one exists], `imageapplycolormatrix_va.md`, `refcache_per_camera_isp.md`, etc.). Introduced by v2.1.1 doc-hygiene commit (`398eca7`) — agent-generated citations that were not verified against disk. | **Cleaned.** 8 phantoms → mapped to closest-matching actual-file citation: `composite_anchor_n1_reducer.md`→`composite_producer.md` (audit-confirmed same material), `demosaicv1_jit_kernel.md`→`demosaicv1_details_cleanup.md`, `ig_offset10_consumer.md`→`per_camera_radiometric_weight.md`, `ig_offset10_scalar.md` removed (content was duplicated from per_camera_radiometric_weight), `imageapplycolormatrix_va.md`→`ccm_factory_to_runtime_transformation.md`, `profile2_camera_characterization.md` removed (no md alternative; profile=2 material is in `profile2_*.log` files), `sourceimagecache_writer.md`→`refcache_per_camera_isp.md` (SIC subtree material covered there), `vignetting_runtime_corroboration.md`→`color_pipeline_audit.md`. |
| **OPEN-COMPOSITE-COMBINER** | NEW in v2.1.4 §8.4 — flagged but not investigated | **CLOSED-NEGATIVE.** Literature audit of scratch corpus (`/tmp/l16_open_audit/composite_combiner_audit/scratch_file_summary.csv`) confirmed via 9-stage custody chain that no dedicated multi-cam combiner kernel exists inside libcp.dylib. The 10-cam merge happens OUTSIDE libcp (most plausibly `libImageAPI.dylib`'s `ImageGenerator<uint16_t>::getTile()`). All intra-libcp candidates refuted. §4 row added with closed-negative marker. §2.1 M13 refined with "src1/src2 wrap an external-producer IG" wording. |
| **§2.1 M2.2 NEW** | — | PipelineCache offsets for IRAMP-args decoded from disasm at `libcp+0x3ec7ac..0x3ec7da`: `+0x1e8=scale, +0x238=src1, +0x248=src2, +0x258=&warps, +0x270=&srcs`. Plus aux shared_ptr candidate slots at `+0x1d8/+0x1e0` per PipelineCache dtor. |
| **§2.1 M2.3 NEW** | — | Aux-image characterization: 64-byte POD struct with `width=4160, height=3120` packed Vec2<int32> at +0x08/+0x10/+0x18, `data` ptr at +0x20; 52 MB f32 sensor-native buffer, stride=4160 floats; PipelineCache-owned; written ONCE upstream of IRAMP setup (0 WP trips across 300 invocations); values `[692.75, 739.13]` NOT a radial LUT (non-monotonic, non-symmetric). WarpField math reinterpretation: aux acts as per-pixel scalar modulator of projective row coefs. |
| **§4 new OPEN items** | — | **OPEN-AUX-WRITER** (P1 — blocks Phoenix clean-room; non-blocking for spike with LLDB-dump path). **OPEN-CCM-NORMALIZATION** (non-blocking for spike — rownorm workaround in `spike_v2/src/utils.py:291`). |
| **v2.1.5 artifacts** | — | `/tmp/l16_open_audit/composite_combiner_audit/` (literature audit); `/tmp/l16_open_audit/aux_image_decode/` (aux runtime probe + dump); `/tmp/l16_open_audit/warpfield_decode/` (WarpField byte-layout probe from v2.1.4 — refd here); `/tmp/l16_open_audit/aux_dump/` (spike unblock — one-shot aux-dump probe). Spike workspace: `/Users/ryaker/Dev/L16_Lumen_ReverseEngineering/spike_v2/`. |
| **Open items state** | 17 RESOLVED / 1 PARTIAL / 1 NEW OPEN | **17 RESOLVED / 1 PARTIAL (#10) / 1 CLOSED-NEGATIVE (OPEN-COMPOSITE-COMBINER) / 2 NEW OPEN (OPEN-AUX-WRITER P1, OPEN-CCM-NORMALIZATION non-blocking).** 28mm spike visual-gate reachable via aux-dump path. Phoenix clean-room end-to-end gated on OPEN-AUX-WRITER + OPEN-CCM-NORMALIZATION + libImageAPI.dylib disassembly. |
| **KMS entries added** | — | `1ae9fc97` (IRAMP PipelineCache offsets + aux-image characterization fact); `31ead06c` (OPEN-COMPOSITE-COMBINER closed-negative insight + phantom citation rot report); `97fc48cb` (token-conservation pattern — prefer deterministic scripts over agent spawns); `0d462b0b` (WarpField byte layout fact); `42914b2f` (memoir availability). |
