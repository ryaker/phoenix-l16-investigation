...
if [[ $# -lt 5 ]]; then
  echo "usage: $0 LRI LABEL ENTRY_VA RETURN_VA X,Y [X,Y ...]" >&2
  exit 2
fi
...
SCRIPT="$OUT/prediction_callsites.lldb"
LOG="$OUT/prediction_callsites.log"

mkdir -p "$OUT"
rm -f "$OUT/prediction_callsites.json"
python3 - "$PROBE" "$OUT" "$LRI" "$LABEL" "$SCRIPT" "$ENTRY_VA" "$RETURN_VA" "$@" <<'PY'
import json
import sys
from pathlib import Path

probe, output, lri, label, script, entry, ret, *raw_targets = sys.argv[1:]
...
commands = [
    ...
    f"breakpoint set --shlib libcp.dylib --address 0x{int(entry, 0):x}",
    f"breakpoint set --shlib libcp.dylib --address 0x{int(ret, 0):x}",
    ...
    "script monofusion_flow_origin_probe.configure_prediction_targets(None, " + repr(targets) + ")",
    ...
    json.dumps(str(Path(output) / "prediction_callsites.json")) + ")",
    "quit",
]
...
test -s "$OUT/prediction_callsites.json"
python3 - "$OUT/prediction_callsites.json" <<'PY'
import json
import sys
