...
SCRIPT="$OUT/stages.lldb"
LOG="$OUT/stages.log"

mkdir -p "$OUT"
rm -f "$OUT/stages.json"
...
commands = [
    ...
    "breakpoint set --shlib libcp.dylib --address 0x19955e",
    "breakpoint set --shlib libcp.dylib --address 0x199840",
    "breakpoint set --shlib libcp.dylib --address 0x1998e9",
    "breakpoint set --shlib libcp.dylib --address 0x199995",
    "breakpoint set --shlib libcp.dylib --address 0x199af0",
    "script monofusion_flow_origin_probe.attach_stages(lldb.debugger, "
    "{'producer_entry': 1, 'stage_19955e': 2, 'stage_199840': 3, "
    "'stage_1998e9': 4, 'stage_199995': 5, 'stage_199af0': 6})",
    ...
    json.dumps(str(Path(output) / "stages.json")) + ")",
    "quit",
]
...
test -s "$OUT/stages.json"
python3 "$ROOT/tools/lldb_probes/prefusion_monofusion_flow_origin/verify_stage_capture.py" \
  "$OUT/stages.json"
