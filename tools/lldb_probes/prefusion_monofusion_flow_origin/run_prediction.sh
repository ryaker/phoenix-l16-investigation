...
SCRIPT="$OUT/prediction.lldb"
LOG="$OUT/prediction.log"

mkdir -p "$OUT"
rm -f "$OUT/prediction.json"
...
commands = [
    ...
    "breakpoint set --shlib libcp.dylib --address 0x193c50",
    "breakpoint set --shlib libcp.dylib --address 0x194221",
    "script monofusion_flow_origin_probe.attach_prediction(lldb.debugger, 2, 3, 1)",
    ...
    json.dumps(str(Path(output) / "prediction.json")) + ")",
    "quit",
]
...
test -s "$OUT/prediction.json"
python3 - "$OUT/prediction.json" <<'PY'
import json
import sys
...
returns = {item["sequence"]: item for item in report["prediction_returns"]}
for entry in report["prediction_entries"][:8]:
    result = returns[entry["sequence"]]
    print("prediction", entry["grid"], "scale", entry["scale_r8"],
          "previous", entry["previous_flow"]["size"], "result", result["output"])
PY
