...
arch -x86_64 lldb -b \
  -o "command script import $ROOT/tools/lldb_probes/normalization_black_level/black_probe7.py" \
  -o "script black_probe7.reset('$OUT/order.json')" \
  ...
  -o "script black_probe7.attach(lldb.debugger)" \
  -o "process launch -- \"$SRC\" \"$OUT/out7.hdr\" --export-fmt 3 --no-lris" \
  -o "quit" > "$OUT/run7.log" 2>&1
echo "rc=$? label=$LABEL"
