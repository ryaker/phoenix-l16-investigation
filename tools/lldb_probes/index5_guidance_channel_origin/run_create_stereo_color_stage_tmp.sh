#!/usr/bin/env bash
# Variant of run_create_stereo_color_stage.sh for sessions where the lldb-spawned
# process cannot access /Volumes (TCC): stages ALL outputs under /private/tmp,
# then copies results back into the durable runs/ tree and removes the tmp dir.
set -euo pipefail
...
CAMERA_KEY="${3:-0}"
SINGLE_CAMERA="${4:-0}"
OUT="/private/tmp/cs_$LABEL"
DEST="$ROOT/runs/index5_guidance_channel_origin/create_stereo_color_$LABEL"
...
rm -f "$OUT/output.hdr"
cp -f "$OUT"/report.json "$OUT"/run.log "$OUT"/capture.lldb "$DEST"/ 2>/dev/null || true
for f in pre_collapse.f32 post_collapse.rgba32f post_softisp.rgba32f pre_color.rgba32f post_color.rgba32f packed_u8.rgba8; do
  [[ -f "$OUT/$f" ]] && cp -f "$OUT/$f" "$DEST/$f"
done
rm -rf "$OUT"
python3 - "$DEST/report.json" <<'PY'
import json
import sys
...
print(
    ...
    report["packed_u8"]["artifact"]["sha256"],
)
PY
