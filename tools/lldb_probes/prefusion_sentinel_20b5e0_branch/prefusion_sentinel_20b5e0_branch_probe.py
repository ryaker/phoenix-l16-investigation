...
def reset(
    ...
    arm_limit=3,
    watch_hit_cap=128,
    branch_trace_limit=3,
    step_cap=800000,
):
    builtins.l16_prefusion_sentinel_20b5e0_branch = {
        "label": label,
...
def _state():
    if not hasattr(builtins, "l16_prefusion_sentinel_20b5e0_branch"):
        reset()
    return builtins.l16_prefusion_sentinel_20b5e0_branch
...
def _disable_watchpoints(debugger, reason):
    target = debugger.GetSelectedTarget()
...
def install_breakpoints(debugger):
    ...
    for site, name in ((STORE_Y, "store_y_21b92a"), (AFTER_STORE_Y, "after_store_y_21b930")):
        ...
        if not bp or not bp.IsValid():
            ...
            continue
        bp.SetScriptCallbackFunction("prefusion_sentinel_20b5e0_branch_probe.hit")
        state["breakpoint_ids"][name] = bp.GetID()
    print("L16_PREFUSION_SENTINEL_20B5E0_BRANCH_INSTALLED", json.dumps(state["breakpoint_ids"], sort_keys=True))
...
def _trace_20b5e0_branch(thread, sample):
    ...
    if state["counts"]["branch_traces"] >= state["branch_trace_limit"]:
        _disable_watchpoints(process.GetTarget().GetDebugger(), "watchpoints_disabled_after_branch_trace_limit")
    return packet
...
def _record_watchpoint_stop(debugger):
    ...
    if state["counts"]["watchpoint_hits"] >= state["watch_hit_cap"]:
        _disable_watchpoints(debugger, "watchpoints_disabled_after_cap")
...
def report_to_file(debugger, path):
    ...
    with open(path, "w", encoding="utf-8") as fh:
        ...
        fh.write("\n")
    print("L16_PREFUSION_SENTINEL_20B5E0_BRANCH_JSON", path)
