...
SITES = {
    0x3BD308: "case2_target",
    0x3BD31D: "case2_append_after_helper",
    0x3BF2F0: "helper_entry",
    0x3BF331: "helper_optional_3b5b50_call",
    0x3BF344: "helper_call_3b6070",
    0x3BF354: "helper_call_3b07c0",
    0x3BF382: "helper_call_3ba0a0",
    0x3BF39A: "helper_call_3b9820",
    0x3BF3BE: "helper_imagepyramid_ctor",
    0x3BF419: "helper_owner_5a0_callback_call",
    0x3BF481: "helper_owner_640_callback_call",
    0x3BF49A: "helper_completion_flag_write",
    0x3BF4B8: "helper_return",
    0x3BF4C7: "helper_invalid_request_error",
    0x3BF50F: "helper_bad_function_640",
    0x3BF55A: "helper_bad_function_5a0",
}


def reset(label="", sample_cap_per_site=24):
    builtins.l16_codex_final_compositing_case2_helper = {
        "label": label,
...
def _state():
    if not hasattr(builtins, "l16_codex_final_compositing_case2_helper"):
        reset()
    return builtins.l16_codex_final_compositing_case2_helper
...
def _owner(process, target, addr):
    if not addr:
        return None
    ptrs = {
        "ptr_0x0c8": _u64(process, addr + 0xC8),
        "ptr_0x508": _u64(process, addr + 0x508),
        "ptr_0x538": _u64(process, addr + 0x538),
        "ptr_0x5a0": _u64(process, addr + 0x5A0),
        "ptr_0x640": _u64(process, addr + 0x640),
        "ptr_0x688": _u64(process, addr + 0x688),
        "ptr_0x8a0": _u64(process, addr + 0x8A0),
    }
    return {
        "addr": addr,
        ...
        **ptrs,
        "ptr_0x5a0_libcp_va": _libcp_target(target, ptrs["ptr_0x5a0"]),
        "ptr_0x640_libcp_va": _libcp_target(target, ptrs["ptr_0x640"]),
    }
...
def _packet(frame, process, site_va):
    ...
    if site_va in (0x3BD308, 0x3BD31D):
        packet["case2_owner_r15"] = _owner(process, target, regs["r15"])
        packet["case2_record_r13"] = _record(process, regs["r13"])
        packet["local_vector_rbp_minus_0x100"] = _vector(process, regs["rbp"] - 0x100)
    elif site_va == 0x3BF2F0:
        packet["helper_owner_rdi"] = _owner(process, target, regs["rdi"])
        packet["helper_record_rsi"] = _record(process, regs["rsi"])
    else:
        packet["helper_owner_r13"] = _owner(process, target, regs["r13"])
        packet["helper_record_r14"] = _record(process, regs["r14"])

    if site_va == 0x3BF419:
        packet["owner_5a0_callback_target_rax_libcp_va"] = _libcp_target(target, regs["rax"])
        packet["owner_5a0_callback_args"] = {
            "rdi": regs["rdi"],
            "rsi": regs["rsi"],
            "rdx": regs["rdx"],
            "rcx": regs["rcx"],
        }
    elif site_va == 0x3BF481:
        target_ptr = _u64(process, regs["rax"] + 0x30)
        packet["owner_640_callback_vtable_rax"] = regs["rax"]
        packet["owner_640_callback_target_u64_rax_plus_0x30"] = target_ptr
        packet["owner_640_callback_target_libcp_va"] = _libcp_target(target, target_ptr)
        packet["owner_640_callback_args"] = {
            "rdi": regs["rdi"],
            "rsi": regs["rsi"],
            "rdx": regs["rdx"],
            "rcx": regs["rcx"],
        }
    elif site_va == 0x3BF49A:
        completion_ptr = _u64(process, regs["r14"] + 0x28)
        packet["completion_ptr_record_plus_0x28"] = completion_ptr
        packet["completion_ptr_current_u8"] = _u8(process, completion_ptr)
...
def install_breakpoints(debugger):
    ...
    print("L16_CODEX_FINAL_CASE2_HELPER_BPS", json.dumps(state["breakpoint_ids"], sort_keys=True))
...
def drive_until_exit_or_step_cap(debugger, step_cap=120000):
    ...
    print("L16_CODEX_FINAL_CASE2_HELPER_DRIVE_STEPS", steps)
...
def write_report(debugger, path):
    with open(path, "w", encoding="utf-8") as handle:
        ...
        handle.write("\n")
    print("L16_CODEX_FINAL_CASE2_HELPER_REPORT", path)
