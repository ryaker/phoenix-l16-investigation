...
def reset(label="", watch_hit_cap=256, step_cap=4096):
    builtins.l16_c6_postmutation_active_byte_watch = {
        "label": label,
...
def _state():
    if not hasattr(builtins, "l16_c6_postmutation_active_byte_watch"):
        reset()
    return builtins.l16_c6_postmutation_active_byte_watch
...
def install_breakpoints(debugger):
    ...
    if not bp or not bp.IsValid():
        ...
        return
    bp.SetScriptCallbackFunction("c6_postmutation_active_byte_watch_probe.mutation_after")
...
def mutation_after(frame, bp_loc, _dict):
    ...
    active_addr = packet["active_addr_item_0x30"]
    arm = {
        "site": "0x3c90a9",
    ...
    error = lldb.SBError()
    wp = target.WatchAddress(active_addr, 1, True, True, error)
...
def _record_watchpoint_stop(debugger):
    ...
    for packet in state["armed"]:
        if packet.get("watchpoint_id") == wp_id:
            meta = packet
            break
    item = (meta or {}).get("item", {}).get("item_ptr")
    active_addr = (meta or {}).get("item", {}).get("active_addr_item_0x30")
    sample = {
        "watchpoint_id": wp_id,
...
def drive_until_exit_or_step_cap(debugger):
    ...
    print("L16_C6_POSTMUTATION_ACTIVE_BYTE_WATCH_DRIVE_STEPS", steps)
