...
def _state():
    if not hasattr(builtins, "l16_payload_watch"):
        reset()
    return builtins.l16_payload_watch
...
def _install_breakpoints(debugger, names):
    ...
    for va, name in SITES.items():
        ...
        if not bp or not bp.IsValid():
            ...
            continue
        bp.SetScriptCallbackFunction("payload_watch_probe.hit")
        ids[name] = bp.GetID()
...
def attach(debugger):
    ...
    print("L16_PAYLOAD_WATCH_ATTACHED", ids)
...
def _record_watchpoint_stop(debugger):
    ...
    for thread in process:
        ...
        sample = {
            ...
            "stack": _stack(thread),
            "watched_bytes_now_hex": _bytes_hex(
                process, meta.get("watch_addr", 0), meta.get("watch_size", 8)
            ),
        }
...
def drive_until_exit_or_step_cap(debugger, max_steps=12000):
    ...
    print("L16_PAYLOAD_WATCH_DRIVE_STEPS", steps)
...
def write_report(debugger, path):
    with open(path, "w", encoding="utf-8") as handle:
        ...
        handle.write("\n")
    print("L16_PAYLOAD_WATCH_WROTE", path)
