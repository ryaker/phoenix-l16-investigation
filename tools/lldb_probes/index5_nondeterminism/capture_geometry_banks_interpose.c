...
typedef void (*builder_fn)(void *, void *, int32_t);

static uintptr_t libcp_base;
static void *builder_trampoline;
...
static void append_packet(const void *object, const void *record) {
    const char *path = getenv("L16_GEOMETRY_BANK_REPORT");
    if (path == NULL || object == NULL || record == NULL) return;
    uint32_t camera_id;
    ...
    memcpy(factory, (const uint8_t *)object + 0x180, sizeof(factory));
    memcpy(built, record, sizeof(built));
    ...
    if (output != NULL) {
        ...
        fwrite(built, sizeof(built), 1, output);
        fclose(output);
    }
...
static void builder_hook(void *record, void *object, int32_t selector) {
    uintptr_t caller = (uintptr_t)__builtin_return_address(0);
    ((builder_fn)builder_trampoline)(record, object, selector);
    if (caller == libcp_base + 0x3ff1bc && selector == 1)
        append_packet(object, record);
}

__attribute__((constructor))
static void install_builder_hook(void) {
    libcp_base = find_libcp_base();
    if (libcp_base == 0) return;
    builder_trampoline = install_hook(0x264270, 15, (const void *)&builder_hook);
}
