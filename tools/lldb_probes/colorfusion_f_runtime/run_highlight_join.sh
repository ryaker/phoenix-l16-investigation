...
OUTPUT=${CF_RENDER_OUTPUT:-/private/tmp/codex_cf_highlight.hdr}

arch -x86_64 lldb -b \
  -o "target create $ROOT/tools/lri_process" \
  -o "command script import $ROOT/tools/lldb_probes/colorfusion_f_runtime/highlight_join_probe.py" \
  ...
  -o "script highlight_join_probe.install(lldb.debugger)" \
  -o continue \
  -o continue \
  -o quit
