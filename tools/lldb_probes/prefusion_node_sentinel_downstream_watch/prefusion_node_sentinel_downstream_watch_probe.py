...
def reset(label="", sample_limit=128, arm_limit=3, watch_hit_cap=64, step_cap=300000):
    builtins.l16_prefusion_node_sentinel_downstream_watch = {
        "label": label,
...
def _state():
    if not hasattr(builtins, "l16_prefusion_node_sentinel_downstream_watch"):
        reset()
    return builtins.l16_prefusion_node_sentinel_downstream_watch
...
def install_breakpoints(debugger):
    ...
    for site, name in ((STORE_Y, "store_y_21b92a"), (AFTER_STORE_Y, "after_store_y_21b930")):
        ...
        if not bp or not bp.IsValid():
            ...
            continue
        bp.SetScriptCallbackFunction("prefusion_node_sentinel_downstream_watch_probe.hit")
        state["breakpoint_ids"][name] = bp.GetID()
    print("L16_PREFUSION_NODE_SENTINEL_DOWNSTREAM_WATCH_INSTALLED", json.dumps(state["breakpoint_ids"], sort_keys=True))
...
def report_to_file(debugger, path):
    ...
    with open(path, "w", encoding="utf-8") as fh:
        ...
        fh.write("\n")
    print("L16_PREFUSION_NODE_SENTINEL_DOWNSTREAM_WATCH_JSON", path)
