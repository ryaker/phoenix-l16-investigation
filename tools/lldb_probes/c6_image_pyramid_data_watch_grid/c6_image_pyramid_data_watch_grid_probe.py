...
def reset(
    ...
    builtins.l16_c6_image_pyramid_data_watch_grid = {
        "label": label,
...
def _state():
    if not hasattr(builtins, "l16_c6_image_pyramid_data_watch_grid"):
        reset()
    return builtins.l16_c6_image_pyramid_data_watch_grid
...
def _descriptor(process, addr):
    ...
    buffer_bytes = None
    if stride_pixels is not None and height is not None:
        buffer_bytes = stride_pixels * height * 4
    return {
        "addr": addr,
        ...
        "buffer_bytes_assuming_bpp4": buffer_bytes,
        "sample_32_at_data_ptr": _sample(process, data_ptr, 32),
    }


def _aligned_offset(value, alignment=8):
    if value <= 0:
        return 0
    return value & ~(alignment - 1)


def _watch_offset(descriptor):
    state = _state()
    buffer_bytes = descriptor.get("buffer_bytes_assuming_bpp4") if descriptor else None
    watch_size = state["watch_size"]
    if not buffer_bytes or buffer_bytes < watch_size:
        return None
    kind = state["range_kind"]
    if kind == "first":
        return 0
    if kind == "middle":
        return _aligned_offset((buffer_bytes - watch_size) // 2, watch_size)
    if kind == "last":
        return _aligned_offset(buffer_bytes - watch_size, watch_size)
    state["errors"].append({"error": "unknown range_kind", "range_kind": kind})
    return None
...
def install_breakpoint(debugger):
    ...
    if not bp or not bp.IsValid():
        ...
        return
    bp.SetScriptCallbackFunction("c6_image_pyramid_data_watch_grid_probe.zero_fill_after")
...
def zero_fill_after(frame, bp_loc, _dict):
    ...
    offset = _watch_offset(descriptor)
    watch_addr = data_ptr + offset if data_ptr and offset is not None else None
    packet = {
        "site": "0x3b2f59",
        "level": level,
        "range_kind": state["range_kind"],
        "descriptor": descriptor,
        "watch_offset_bytes": offset,
        "watch_address": watch_addr,
        "watch_size": state["watch_size"],
        "watched_bytes_at_arm": (
            _sample(process, watch_addr, state["watch_size"]) if watch_addr else None
        ),
    ...
    if watch_addr:
        error = lldb.SBError()
        wp = target.WatchAddress(watch_addr, state["watch_size"], True, True, error)
        ...
        packet["watchpoint_id"] = None
        packet["watchpoint_error"] = "missing watch address"
        state["errors"].append(packet)
...
def drive_until_exit_or_step_cap(debugger):
    ...
    print("L16_C6_IMAGE_PYRAMID_DATA_WATCH_GRID_DRIVE_STEPS", steps)
